...
public class DoiConstant {
	...
	public static Map<String, Object> buildPageInMapCommonEmp(PageDetails pageDetail) {
		...
		if (searchParams != null && !searchParams.isEmpty()) {
			searchParams
					.forEach(search -> map.put(DoiConstant.getCommonEmpList().get(search.getKey()), search.getValue()));
		} else {
			DoiConstant.getCommonEmpList().forEach((k, v) -> map.put(DoiConstant.getCommonEmpList().get(k), 0));
		}
		return map;
	}
	...
	public static Map<String, Object> buildPageInMapJoiningTransfer(PageDetails pageDetail) {
		...
		map.put(Constant.IN_PAGE_SIZE, pageDetail.getPageElement());
		if (searchParams != null && !searchParams.isEmpty()) {
			searchParams.forEach(search -> map.put(DoiConstant.getSearchJoiningEmployeeMap().get(search.getKey()),
					search.getValue()));
		} else {
			DoiConstant.getSearchJoiningEmployeeMap()
					.forEach((k, v) -> map.put(DoiConstant.getSearchJoiningEmployeeMap().get(k), 0));
		}
		return map;
	}
